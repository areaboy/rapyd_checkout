...
session_start();
//set session
if(!isset($_SESSION['cus_id']) || (trim($_SESSION['cus_id']) == '')) {
...
<li class="navbar-brand home_click imagelogo_li_remove" ><img class="img-rounded imagelogo_data" src="img/bike-img.jpeg"></li>
                ...
                <a style='display:none;' class="navbar-brand" href="index.php">Home/Shop</a>
...
<h3><center><b style='color:'>View Your Purchase and Payments</b></center></h3>
...
$res= $db->prepare("SELECT count(*) as totalcount FROM sales where email=:email and payment_status='CLO' ");
$res->execute(array(':email' =>$cus_email));
$t_row = $res->fetch();
$totalcount = $t_row['totalcount'];

if($totalcount == 0){
echo "<div style='background:red;color:white;padding:10px;border:none;'>You have not make any Purchase and Payments Yets.<b></b></div>";
//exit();
}

$result = $db->prepare("SELECT * FROM sales where email=:email and payment_status='CLO' order by id desc limit :row1, :rowpage");
$result->bindValue(':rowpage', (int) trim($rowpage), PDO::PARAM_INT);
$result->bindValue(':row1', (int) trim($limit), PDO::PARAM_INT);
$result->bindValue(':email', trim($cus_email), PDO::PARAM_STR);
...
<th> <font face="Arial">Payments Link Reminder</font> </th> 
...
?>
                  ...
                  <td>
<a target='_blank' href='<?php echo $payment_redirect_url; ?>' title='Click to Complete Payments' style='font-size:12px;' class="email_css_no">
 Click to Complete Payments</a></td>
...
<b>Shiiping Address: </b><span class='p_addressx'></span><br>
...
<input class="secured_pot" type="text" name="emailaddress_pot" id="emailaddress_pot" />
                    ...
                    <input type="button" id="save_btn" class="pull-right btn btn-primary" value="Create Products" />
                </form>
