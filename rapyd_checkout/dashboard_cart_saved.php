...
<h3><center><b style='color:'>Customers Saved Carts For Later Purchase Tracking & Email Reminding System....</b></center></h3>
Easily track and send email reminders to Customers  urging them to checkout their saved carts and make payments...
...
$res= $db->prepare("SELECT count(*) as totalcount FROM cart where cart_status='saved'");
$res->execute(array());
$t_row = $res->fetch();
$totalcount = $t_row['totalcount'];

if($totalcount == 0){
echo "<div style='background:red;color:white;padding:10px;border:none;'>No Carts Saved by Your Customers Yet for Later Checkout and Payments.<b></b></div>";
//exit();
}

$result = $db->prepare("SELECT * FROM cart where cart_status='saved' order by id desc limit :row1, :rowpage");
...
<th> <font face="Arial" style="color:red">Reason for later Payment</font> </th> 
...
<td><div class='<?php echo $colorx; ?>' > <?php echo $st; ?> </div></td> 
                  ...
                  <td style='color:red'><?php echo $reason; ?></td> 
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Customers Saved Cart Email Reminders</h4>
...
$('#loader_rec').fadeIn(400).html('<br><div style="color:black;background:#ddd;padding:10px;"><img src="loader.gif" style="font-size:20px"> &nbsp;Please Wait, Saved Cart Reminder is being sent to Customers Email.</div>');
var datasend = {email_reminder:email_reminder, cart_session:cart_session, product_id:product_id, cart_id:cart_id,email:email,fullname:fullname};


$.ajax({
			
			type:'POST',
			url:'checkout_saved_carts_reminders.php',
			data:datasend,
                        crossDomain: true,
			cache:false,
...
kindly complete your Saved Checkout and Make Payments with Link send to your Email Address... </textarea>
...
<button type="button" id="email_btn" class="btn btn-primary" title='Send Email Reminders'>Send Email Reminder</button>
</div>
